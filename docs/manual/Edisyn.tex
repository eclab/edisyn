...
{\large \bf A Java-based Synthesizer Patch Editor, Version 24\\[0.2em]
...
\item Casio CZ Series (Single)
\item Dave Smith Instruments Prophet '08 and '08 Desktop, Tetra, Mopho, Mopho Keyboard and SE, and Mopho X4 (Single and (for Tetra) Combo Modes)
...
\item Yamaha DX7 Family (DX7, TX7, TX216/816, Korg Volca FM, Dexed, DX200, DX9, FS1R, etc.) (Single)
...
\item Yamaha FS1R (Voice and Performance Modes)
\item Yamaha TG33, SY22, and SY35 (Single and (for TG33) Multimode)
\item General CC, NRPN, and RPN editing
\item Microtuning editing
\end{itemize}
}

You'll note a pattern among these synthesizers: many are very difficult to program.\footnote{So why are the Prophet '08, Blofeld, and Microwave XT there then?  Because in fact these synthesizers are all ones I own or have had access to.}  That's one of my interests.  

Edisyn's patch editors try to cover the most common needs, and so it does not support patches for global parameters, nor for wave, sample, or wavetable editing\footnote{The Blofeld patch editor only supports wavetable {\it uploading}, not {\it editing}: this is because there's an excellent free wavetable-editing tool available at https:/\!/synthtech.com/waveedit which is better than anything I could possibly write.} etc.  Additionally, though it can do bulk downloads and save them as individual patches, Edisyn is {\bf not at present a librarian tool}.  You should use a good free librarian software program, such as SysEx Librarian on the Mac.
...
Edisyn tends to work with files which contain a single patch each.   However many patch files on the internet are what I call {\it bulk sysex} files, and others are {\it bank sysex} files.  

A bulk sysex file contains a bunch of individual patch sysex messages concatenated together\,---\,you could just cut it up into separate single-patch files.  Bulk sysex files are common in many synthesizers, such as the DSI Prophet '08 or the Yamaha FS1R.  When Edisyn loads a bulk sysex file, it will present you with two primary options: you can {\it select and edit} a single patch from that file, either in the current editor or a new editor, or you can {\it upload} all of the patches in the file to your synthesizer.

It is possible that a bulk sysex file contains patch entries for different kinds of editors.  For example, a bulk sysex file might contain entries for both single patches and multi patches, and these are loaded into different editors.  Edisyn will inform you when there are patches for different editors involved, and let you select first which editor you're interested in, then let you select which patch you want to edit.  Also, when Edisyn presents you with a collection of bulk sysex patches, they'll be numbered 1 ... \(n\).  This doesn't mean that they'll be {\it written} to slots 1...\(n\) on your synthesizer: it's just the order that they appear in the file.

Other synthesizers have special {\it bank sysex} messages which contain an entire bank (or in some cases, an entire synthesizer's memory worth) of patches.  These messages could be stored in files or be dumped to Edisyn from the synthesizer itself.  Only a few Edisyn patch editors know how to deal with these messages: notably the DX7 patch editor can handle bank messages just fine.  When Edisyn has received a bank sysex message or loaded one from a file, you will be given three options, to {\it upload} the entire bank to the synthesizer, to {\it save} the entire bank to a file, or to {\it select} a single patch from the bank and edit it.

It's entirely possible that a bulk sysex file might itself contain multiple bank sysex messages.  If from the Bulk Sysex window you select one to load into the editor, Edisyn will then present you with the Bank Sysex window.  Note that in the Bulk Sysex window, these sysex messages will be simply labelled ``Bank Sysex'', since Edisyn has no other patch name for them.

Edisyn cannot presently construct (and thus save or upload) new bank sysex messages or files.
...
\vspace{-2em}\caption{Editing Mutation Parameters}\label{mutationparameters}
\end{wrapfigure}
...
Red Sound DarkStar/DarkStar XP2&&&&&{\cm}&&{\cm}& & &            \\
