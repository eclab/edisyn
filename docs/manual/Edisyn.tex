...
{\large \bf A Java-based Synthesizer Patch Editor, Version 15\\[0.2em]
...
\item Yamaha DX7 Family (DX7, TX7, TX216/816, etc.) (Single)
...
\begin{figure}[t]
\begin{center}
\includegraphics[width=6in]{HillClimbing.pdf}
\end{center}
\caption{Recombination and Opposite-Recombination in the Hill-Climber. Shown are (left to right) basic operations performed when the user has made three (A, B, C), two (A, B), or only one (A) selections.  Z is the top selection the previous iteration (the previous iteration's A).  Beyond the given operations, various degrees of mutation are applied to each new child: see Figure \ref{hctable}.}
\label{hcfigure}
\end{figure}
...
Edisyn's {\it hill-climber}\footnote{Important Note.  The Hill Climber is a work in progress and as testing and research continues on it, it may change significantly} is another of its patch-exploration tools.  You select it by choosing {\bf Hill-Climb} in the {\bf Edit} menu.  This will add a new tab to your patch editor labelled {\bf Hill-Climb}.   When you fire up the Hill-Climber, it appears as an additional tab after your {\bf About} tab.  Whenever you select a tab other than the hill-climber tab, the hill-climber will pause; when you go back to the hill-climber tab it will resume.  You get rid of the hill-climber by selecting {\bf Stop Hill-Climbing} in the {\bf Edit} menu. 

Edisyn's Hill-Climber repeatedly offers you sixteen new versions of patches and asks you to choose your top three preferences: you can also indicate preference for your current working patch or for nothing.  After you have chosen them, the Hill-Climber will try to build a new set of sounds whose parameters are similar to your choices in various ways.  If you're lucky, Edisyn will head towards regions of the synthesizer's parameter space which make sounds you like.  {\bf Hint:  You will have more success with the hill-climber if you restrict the parameters being mutated to just those you want to explore.}  Different kinds of synthesizers will also benefit from different amounts of exploration (mutation and recombination) rates.  You'll need to tweak those as necessary.

The Hill-Climber builds new sounds by recombining your top three sound choices and adding some degree of noise (mutation) to them.  Let's say that your top three sounds were sounds {\bf A, B,} and {\bf C}, and that your {\it previous} ``A'' from last time is called {\bf Z}.  Figures \ref{hcfigure} and \ref{hctable} explain the mechanism by which the 16 new patches are generated.  The hill-climber tries to balance coming up with mixtures of your selections as well as things well outside the searched space (balancing so-called {\it exploration} and {\it exploitation}). You can also select only two top choices (A and B), a single choice (A), or no choices at all\,---\,in which case Edisyn just generates 16 mutated replacements for the existing ones.
...
\begin{itemize}
\item{1--4}: Four 1x mutated versions of the current patch
\item{5--8}: Four 2x mutated versions of the current patch
\item{9--12}: Four 3x mutated versions of the current patch
\item{13--16}: Four 4x mutated versions of the current patch
\end{itemize}

The sixteen candidates appear in the {\bf Candidates} section.  The option of choosing your current patch is in the {\bf Current} section, and the option of choosing no patch is in the {\bf None} section.  Each option (except None) has a button labelled {\bf Play}, which lets you play the candidate, and each Candidate has a button labelled {\bf Options}, which presents so additional options).  There are also three radio buttons, labelled 1, 2, and 3.  These are for you to select the sound as one of your top three choices, if you like it.
...
\paragraph{Nudging in the Hill-Climber}

If you have loaded patches to nudge (typically via {\bf Nudge:Set ...} in the {\bf Edit} menu) they become available in the hill-climber.  When you select the nudge target (1, 2, 3, or 4 or the Current Patch), and then press {\bf Nudge To:}, then {\bf all of the candidates} will get nudged towards the target.  The amount of nudging that occurs is set by the standard nudge recombination and mutation amounts (see the {\bf Nudge:Set Nudge Mutation} or {\bf Nudge:Set Nudge Recombination} under the {\bf Edit} menu).

By pressing {\bf Go to:} you can also reset all of the candidates with patches formed by essentially reinitializing from the nudge target just as pressing the Reset button would reinitialize from the current patch.  

Note that neither Go to: nor Nudge To: reset the the stack nor increase the iteration: they just change the candidate sounds.  Thus if you afterwards press ``Retry'', the current iteration is retried once again; or if you press ``Back Up", the iteration is backed up as normal.

\paragraph{Candidate Options}

If you press the {\bf Options} button on a candidate, a menu pops up with a variety of options:

\begin{itemize}
\item {\bf Keep Patch}\quad Loads the candidate into the current patch in your patch editor.
\item {\bf Edit Patch}\quad Creates a new patch editor and loads the candidate into that.  Note that if you edit the patch in the patch editor, the {\it changes you make will be automatically reflected in the candidate}.  If you update the candidate (by hill-climbing or nudging or backing up etc.) then this connection will be broken
\item {\bf Save to File}\quad Saves the candidate to a file.
\item {\bf Load from File}\quad Changes the candidate to a patch loaded from a file. 
\item {\bf Set... }\quad Copies the candidate into one of the nudge targets.
\item {\bf Load...}\quad Changes the candidate into a copy of one of the nudge targets. 
\end{itemize}

Note again that, as was the case in nudging above, neither Load from File nor Load... reset the the stack nor increase the iteration: they just change the candidate sounds.  Thus if you afterwards press ``Retry'', the current iteration is retried once again; or if you press ``Back Up", the iteration is backed up as normal.
...
\begin{algorithm}[H]
   \caption{MetricMutate}
   \label{alg:example}
\begin{algorithmic}
   \STATE {\bfseries Input:} Parameter \(P\) with current value \(v\); {\it weight}
   \STATE \(q \leftarrow \text{\it weight} \times\)
   \STATE \hspace{\fill}\((\text{metric max of \(P\)} - (1 + \text{metric min of \(P\)}))\)
   \STATE \(u \leftarrow\) uniform random selection from
   \STATE \hspace{\fill}\([v-q/2, v+q/2]\)
   \IF{\(u=v\)}
   	\IF{coin flip of probability 0.5}
		\REPEAT
			\STATE \(u \leftarrow u + (\text{either 1 or \(-1\) at random})\)
			\STATE \(u \leftarrow\) round \(u\) to nearest metric value in \(P\)
		\UNTIL{coin flip of probability 0.5}
	\ENDIF
   \ENDIF 
...
\begin{algorithmic}
   \STATE {\bfseries Input:} Parameter \(P\), with current value \(v\), {\it weight}
    \IF{\(P\) is fully metric}
    \STATE {\bfseries return } MetricMutate(\(P, v\), {\it weight})
    \ELSIF{\(P\) is semi-metric and \(v\) inside metric region}
   	\IF{coin flip of probability 0.5}
   		 \STATE {\bfseries return } MetricMutate(\(P, v\), {\it weight})
	\ELSIF{coin flip of probability {\it weight}}
		\STATE {\bfseries return} a random non-metric value from \(P\)
	\ENDIF
    \ELSIF{\(P\) is semi-metric and \(v\) outside metric region}
		\IF{coin flip of probability {\it weight}}
		   	\IF{coin flip of probability 0.5}
				\STATE {\bfseries return} a random metric value from \(P\)
		    	\ELSE
	    			\STATE {\bfseries return} a random non-metric value from \(P\)
    			\ENDIF
		\ENDIF
...
\begin{algorithm}[H]
   ...
   \label{alg:example}
\begin{algorithmic}
   \STATE {\bfseries Input:} Parameter \(P\) with current values \(v, w\); {\it weight}
	\IF{coin flip of probability {\it weight}}
	    	\IF{\(v\) and \(w\) are both in metric region in \(P\)}
			\STATE  {\bfseries return } a random selection from \([v, w]\) 
		\ELSIF{coin flip of probability 0.5}
...
\begin{algorithm}[H]
   ...
   \label{alg:example}
\begin{algorithmic}
   \STATE {\bfseries Input:} \hspace{-0.04em}Parameter \(P\) with current values \(v, w\);
   \STATE \hspace{\fill}{\it weight}, {\it flee}
 	\IF{\(v=w\) and {\it flee}\(=\){true}}
		\IF{\(v\) is non-metric}
			\IF{coin flip of probability {\it weight}}
    				\STATE {\bfseries return } a random non-metric value from 
				\STATE \hspace{\fill} \(P - \{w\}\)
			\ENDIF
		\ELSIF{\(v=\) metric max of \(P\)}
			\STATE {\bfseries return } \(v - 1\)
		\ELSIF{\(v=\) metric min of \(P\)} 
			\STATE {\bfseries return } \(v + 1\)
		\ELSE
			\STATE {\bfseries return } \(v\ +\) or \(- 1\) at random
		\ENDIF
    \ELSIF{both \(v\) and \(w\) are metric values in \(P\)}
    	\STATE \(q \leftarrow v + \text{\it weight} \times (v - w)\)
    	\STATE \(q \leftarrow \text{min}(\text{max}(q, \text{metric min of \(P\)}), \text{metric max of \(P\)})\)
    	\STATE {\bfseries return} a random selection from \([v, q]\) 
    \ELSE
        \STATE {\bfseries return} \(v\)
     \ENDIF
  \end{algorithmic}
