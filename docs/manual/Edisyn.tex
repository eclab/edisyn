...
{\large \bf A Java-based Synthesizer Patch Editor and Librarian, Version 29\\[0.2em]
...
\item Korg Volca Series (Bass, Beats, Drum Single, Drum Split, FM, Keys, Kick, NuBass, Sample/Sample2 Multi, Sample2 Single, /u/pajen firmware)
...
\item Waldorf Blofeld Desktop, Blofeld Desktop SL, and Blofeld Keyboard (Single and Multimode, plus wavetable uploading in Single)
...
Edisyn tends to work with files which contain a single patch each.   However many patch files on the internet contain multiple patches.  There are two kinds of files of this type: what I call {\bf bulk sysex} files, and {\bf bank sysex} files. 
...
\item {\it Save} all of the patches in the file, or patches matching a current synth type, to bulk files (one bulk file per synth type).
\item {\it Save} all of the patches in the file, or patches matching a current synth type, to individual patch files.  If there are multiple synth types, the individual patch files will be placed in separate directories by synth type.
\end{itemize}
...
On some but not all synthesizers, if you change widgets in the patch editor, the synth will automatically update itself.   The opposite sometimes happens as well: changing a parameter on the synthesizer will update it in Edisyn.  See the About pane to determine if your synthesizer can't do this.  Even if your synthesizer can respond to real-time changes, you may wish to turn this feature off by unchecking {\bf Sends Real Time Changes} (your selection is stored in Edisyn per-synthesizer, so different synth editors will check or not check this menu option).
...
\paragraph{Blend}  This causes Edisyn to request two random patches from your synthesizer and merge them with one another, with a merge percentage randomly ranging from 50\% to 100\%, producing an entirely new randomly combined patch.  You could do this manually of course: it's a convenience mechanism.  

See the {\bf Blend} submenu in the {\bf Edit} menu.  You can select two entirely random patches using {\bf Once}, or you can select the patches from a patch range using {\bf Once in Range...} (you'd likely do this to, say, only blend two drum patches or two orchestral patches).   Blend will then do its thing.  If you want to try again, select {\bf Undo and Blend Again}.\footnote{Note that because Blend loads the first patch, then merges the second, it effectively pushes {\it two} things onto the undo stack; so if you want to undo a Blend, you'll need to Undo {\it twice}.  Similarly, Undo and Blend Again will pop {\it two} things off before re-blending.  This might be confusing.}
...
Once you're done choosing parameters to mutate or recombine, just select {\bf Stop Editing Mutation Parameters} in the {\bf Edit} menu.  Note that your parameter choices, as well as using them for recombination, are persistent: they're saved in the preferences.
...
\paragraph{Autopilot}  The morpher can be set to move the cursor randomly on its own to wander about in the space.   There are three parameters which affect this.  First, when the morpher is about to move the cursor, the {\it Autopilot Jump} specifies how far the cursor will be moved.  Second, the {\it Autopilot Smooth} specifies how much the movement will be totally random (jittery) rather than a mixture of moving forward and moving randomly.  Third, the {\it Autopilot Rate} specifies how often the cursor is moved (higher is faster).  Note that if your synthesizer requires significant pauses after sending a  parameter or sending a whole patch, this will significantly slow down the autopilot.  For example, if you are trickling, and the trickle rate is set to 20, then 20 trickles will be sent for each autopilot update, and if a single trickle is slow, the autopilot update rate will be very slow. 
...
If you're editing a DX7, you can use the neural network in hill-climbing by selecting {\bf NN Hill-Climber} in the Hill-Climber's ``Method'' chooser.  Thereafter instead of creating children patches via crossover and mutation, Edisyn will produce them using the autoencoder.  Similarly, if (when editing a DX7) you select {\bf Edit \(\rightarrow\) Randomize \(\rightarrow\) Randomizes Using NN}, then randomization options will use the autoencoder rather than the standard mutator.

\paragraph{What's the point?}  As anyone who programs synthesizer patches knows, the space of synthesizer patches consists of small useful regions surrounded by lots of patches which are garbage, even total silence.  The hill-climber tries many strategies to avoid presenting garbage and silence to the user, but is only partly successful   The goal of the autoencoder is to eliminate as much garbage and silence as possible, thus improving the chances that you'll get decent patch results.  We'd appreciate feedback as to how successful you think we were.
 
\paragraph{How does it work?}  A variational autoencoder is a special kind of neural network trained to take an incoming patch and then output the same exact patch.  The trick, however, is that it has to get it through a narrow neck much smaller than the size of a standard patch: it must squeeze the patch down to a much smaller space, called the {\it latent space} of the autoencoder, in order to get it through.  The autoencoder must learn how to compress all the 30,000 patches fed to it such that when they are decompressed after passing through the narrow neck, they're more or less the same as they were originally.  

In order to compress those 30,000 patches, the neural network has to learn the features more or less common to them.  In short, it tightly squeezes the regions of the patch space that contain those 30,000 patches into the latent space, and in doing so it ignores the features of the regions of the patch space that {\it didn't} contain those 30,000 patches.  What regions of the patch space were that?  Ideally, the useless garbage and silence!  Now the ``compressed'' space, with luck, contains only useful patch regions.

Once we have trained the autoencoder, we can break it into two pieces: the compression portion and the decompression portion.  If we hand a random ``compressed'' vector to the decompression portion, it will decompress it into a patch somewhere in the vicinity of the 30,000 patches\,---\,and {\it not}, ideally, in garbage regions of the space.  So now we can mutate a patch by compressing it into the smaller space, then making some random mutations to its {\it compressed} form, then decompressing it.  And we can hill-climb by wandering around not in the patch space but in the compressed space, decompressing results as needed to audition them.
...
\paragraph{About those 30,000 patches...} In Edisyn's \texttt{resources} directory, we have included a text file containing, one to a line, the DX7 parameters of each of those patches as numbers. Perhaps it might be useful to other researchers. 
...
\hspace{0em}{\color{red}\bf Warning: The Librarian is not ready for prime time.  It has only been tested on a few synthesizers, and is subject to radical change.  You may not  even have it on your version of Edisyn yet.}

Most patch editors in Edisyn sport a simple librarian.  You can call up the librarian by choosing {\bf Open Librarian} from the {\bf Librarian} menu.  The Librarian pops up as its own tab, and looks something like what's shown in Figure~\ref{librarianpanel}, thought the particular layout will vary depending on your synthesizer. The purpose of a librarian is to allow you to load and save, rearrange, and upload or download patches in bulk.  Edisyn's libarians are rudimentary but they should get the job done.  

The Librarian presents itself as a table with columns and rows.  Each column is a {\it bank} in your synthesizer, and each row is a {\it patch number} across all banks.  Edisyn tries to label the banks and the patch numbers like your synthesizer does: for example, in the Roland D-110 [Tone], patch numbers start at 11, so the Librarian does too.  You can rearrange and resize the bank columns.  
...
\begin{itemize}
\item {\it Pale Green: the \textbf{Scratch Bank}}\quad The first column is special.  This isn't a bank in your synthesizer: it's just some extra room for you to temporarily stash stuff as you move things around via drag-and-drop. 
\item {\it White}\quad These are patches in locations which can be written to your synthesizer.
\item {\it Pale Magenta}\quad These are patches in locations which {\bf cannot} be written to your synthesizer because they're read-only on your machine.  However you can save them to disk, and can load them from the disk or from your synth.
\item {\it Light Gray with Red Lettering}\quad These are patches in locations which {\bf cannot} be written to your synthesizer because they're not valid locations.  Why does Edisyn provide them then?  Because some synths have banks with varying lengths or present other unusual situations, and Edisyn has to be consistent.  You can treat them as scratch locations.
\item {\it Dark Gray}\quad These are patches you have selected.
\item {\it Pale Violet}\quad This is the drop location for drag-and-drop.
\end{itemize}
...
\paragraph{Loading Patches into the Librarian from your Synthesizer}
...
If you open a bulk or bank sysex file, you'll be given the option (among other things) to load the data into your current Librarian or into one attached to a new editor.  If the Librarian isn't already open, it'll be opened for you.  Note that many synthesizer patches don't indicate their bank or number: Edisyn will just start filling them in starting at the first bank.  If multiple patches in the same file incorrectly have the same bank and number, Edisyn will put them where it can.  

\paragraph{Requesting Banks}

Normally if you download a range of patches, a bank, or, all the patches, Edisyn will request each patch one by one.  However a few synthesizers are capable of sending entire banks of patches in special sysex messages, and in some cases Edisyn is capable of requesting and reading these.  In these cases, you may see an additional menu item, {\bf Request Bank from Synth}.  Choosing this menu will allow you to select which bank to request, and then Edisyn will request it from the synthesizer.

\paragraph{Where Loaded Patches Go}

If a single patch arrives while the librarian is open, it'll go in the librarian rather than the editor.  If you load a range of patches, the Librarian will try to be smart about it.  If the patches come with locations, the Librarian will put them in their locations.  If they don't, it'll stick them in order starting with your first bank.  

Some synthesizers can send whole banks to Edisyn.   Even so, when loading a bank from a synthesizer, Edisyn will request the patches one by one.  But if Edisyn receives a bank message from a synthesizer or disk, it'll try to be smart about where to put it.  If the bank message indicates which bank it is, Edisyn will put it there.  Otherwise it'll put it in the scratch bank.
...
Once you have patches loaded into your Librarian, you can select a range of patches in a given bank by dragging the mouse over them.  Alternatively you can double-click on the bank name (the column header) and all the patches in the bank will be selected.

After you have selected a patch range, you can drag it from one column to another.  By default, dragging will {\bf move} the patches to the new location, overwriting the patches you dropped them on top of.  If you instead hold down the Control key while dragging (on the Mac you can also hold down the Option key), the you will instead {\bf copy} the patches: copies will move to the new location but the originals will stay put.  Finally if you hold down the Control and Shift keys while dragging, the drag cursor changes to Java's ``link'' cursor:   Edisyn uses this final mode to indicate that you will {\bf swap} the patches with the ones they're being dragged on top of.
...
Java's table facility is weird about selecting and dragging table cells, but the general rule is this: you must first select a patch or patch range, then {\it let go of the mouse,} before you can go back and drag it.


\paragraph{Moving Patches to and from the Editor}

The Editor is completely separate from its Librarian.  Changing a patch in the editor doesn't change it in the Librarian, and loading a new patch in the Librarian doesn't overwrite it in the Editor. 

Instead, the Editor's current patch is shown in the Librarian as the {\bf Current Patch}.  If you want to copy the Editor's current patch to the Librarian, just drag the Current Patch field into the Librarian's table.  Likewise if you want to load a Librarian patch into the Editor, just drag the patch to the Current Patch field.  Additionally you can double-click on a Librarian patch to do the same thing: this will also switch to the Editor for your convenience. 

A common need is to rename the patch.  At present this must be done in the Editor.  First, double-click the patch in the Librarian.  Then change the name in the Editor.  Go back to the Librarian and drag the Current Patch field over the patch in the Librarian table, overwriting it.  (Yes this is cumbersome).
...
\subsection{Writing and Saving} 

\paragraph{Writing Patches to your Synthesizer}

To write a patch or a range of selected patches to your synthesizer, select the range and choose {\bf Write Selected Patches to Synth} from the {\bf Librarian} menu.  This writes the patches as if you had loaded each one in the editor and chosen {\bf Write to Patch...}.  Note that some synthesizers (such as the Yamaha DX7) cannot do this: you can only write whole banks to them.  So....

To write a bank of patches to your synthesizer, select the a patch in the bank and choose {\bf Write Bank} from the {\bf Librarian} menu.  This writes the whole bank of patches to the synthesizer.  On some synthesizers (like the Yamaha DX7) this will use a special bank-writing sysex message. For most synthesizers, it'll just write each patch separately.
...
To save a patch or a range of selected patches to a file, select the range and choose {\bf Save Selected Patches} from the {\bf Librarian} menu.  This will save all the patches to a single bulk file.

To save a bank of patches to a file, select the a patch in the bank and choose {\bf Save Bank} from the {\bf Librarian} menu.  On some synthesizers (like the Yamaha DX7) this will save with a special bank-writing sysex message. For most synthesizers, it'll just write each patch separately to the file.

To save all patches in the Librarian to a file, choose {\bf Save All Patches} from the {\bf Librarian} menu.  On some synthesizers (like the Yamaha DX7) this will save with a special bank-writing sysex message. For most synthesizers, it'll just write each patch separately to the file.

\paragraph{Closing the Librarian}

You can close the Librarian by selecting {\bf Close Librarian} in the {\bf Edit} menu.  The data doesn't go away: if you reopen it, you'll be right back to where you left off.
