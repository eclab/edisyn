...
\noindent Edisyn is a no-nonsense synthesizer patch editor designed to be extensible for many different kind of of synthesizers, and to have unusual programming assistance capabilities.  It is not skewmorphic and not skinnable: its design is plain and consistent.  Edisyn is free open source.

Edisyn at present has patch editors for the following synthesizers: the very ones that I own! 
...
Edisyn does not support patches for global parameters, nor for wave, sample, or wavetable editing etc.  Additionally, though it can do bulk downloads and save them as individual patches, Edisyn is {\bf not a librarian tool}.  You should use a good free librarian software program, such as SysEx Librarian on the Mac.
...
Also note that as a failsafe Edisyn only allows the frontmost window to receive data over MIDI.  This means that (for the moment) while you're batch-downloading, you can't go to some other window (read a web browser say).  The patch editor has to stay in front.\footnote{Yes, I agree it's a stupid restriction.  I'm working on it.}
...
If you don't have a controller keyboard, you can send a test note to your synthesizer by choosing {\bf Send Test Note}.  You can also toggle whether Edisyn constantly sends a stream of test notes by choosing {\bf Send Test Notes}.  And you can shut off all sound on the synthesizer with {\bf Send All Sounds Off}.
...
\begin{wrapfigure}{r}{3in}
\vspace{-1.5em}\includegraphics[scale=0.3]{5.png}
...
{\small {\bf Important Note}\quad {\it The Hill Climber is a work in progress and as testing and research continues on it, it may change significantly.}}

\vspace{1em}
Edisyn's {\it hill-climber} is another of its patch-exploration tools.  You select it by choosing {\bf Hill-Climb} in the {\bf Edit} menu.  This will add a new tab to your patch editor labelled {\bf Hill-Climb}.  You get rid of the hill-climber by selecting {\bf Stop Hill-Climbing} in the {\bf Edit} menu.  
...
The Hill-Climber builds new sounds by recombining your top three sound choices and adding some degree of noise (mutation) to them.  Let's say that your top three sounds were sounds {\bf A, B,} and {\bf C}, and that you kept around your previous ``A'' (\#1 sound) from last time.   Then the 16 new sounds will be (``+'' means ``recombined with''):

\vspace{1em}
\noindent\begin{tabular}{@{}rlrlrlrl@{}}
1 & A & 2 & A + B & 3 & A + C & 4 & A + (B + C) \\
4 & Noisy A & 6 & Noisy A + B & 7 & Noisy A + C & 8 & Noisy A + (B + C)  \\
9 & Very Noisy A & 10 & Very Noisy A + B & 11 & Very Noisy A + C & 12 & Very Noisy A + (B + C) \\
...
In general: your internal parameters should have contiguous ranges and make sense from a user perspective and not the synthesizer's weird parsing perspective.
...
\subsection{Step Six: Get Batch Downloads Working}
...
Edisyn can download many patches at once.  To support this, you need to implement a few methods.\footnote{Until you implement {\bf getPatchLocationName(...)} to return something other than {\tt null}, Edisyn will keep the Batch Downloads menu disabled.  So when you implement this method, be sure to also implement the other methods here at the same time.} First, there's
 {\bf getPatchLocationName(...)}, which returns as a String a short version of the patch address (bank, name) to be used in a saved filename.  Next, there's {\bf getNextPatchLocation(...)} which, given a Model containing a patch address, returns a model with the ``next'' patch address.  Finally you need to implement {\bf patchLocationEquals(...)}, which compares two patches to see if they contain the same patch address.
...
\subsection{Step Eight: Submit Your Patch Editor!}
