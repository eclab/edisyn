...
{\large \bf A Java-based Synthesizer Patch Editor, Version 28\\[0.2em]
...
Edisyn is a no-nonsense synthesizer patch editor for the editing and parameter exploration of a variety of synthesizers.  It is not skeuomorphic or skinnable: its design is plain and consistent, though you can change the colors.  Edisyn is free open source.  Edisyn currently supports the following devices: 
...
An Edisyn patch editor is a single window with multiple tabbed panes.  You can switch tabs by clicking on them or via shortcuts (see the {\bf Tabs Menu}).  The far-right tab is the {\bf About Tab}.  It gives you information about the eccentricities of the synthesizer that require custom behavior in Edisyn (they all do!).  {\bf \color{red} You should read the About Tab carefully to understand how to get Edisyn to properly with your synthesizer.}
...
By selecting {\bf Request Current Patch}, you can ask your synthesizer to send you a dump of whatever patch it is currently running.  In their response, synthesizers  often don't tell Edisyn what the patch number or bank is.  In these cases Edisyn will reset the patch number to some default (like A001).  
...
\vspace{-4em}\caption{Editing Mutation Parameters}\label{mutationparameters}
\end{wrapfigure}
...
\begin{wrapfigure}{r}{3.6in}
%\vspace{-2.65em}
\vspace{-0.75em}\includegraphics[scale=0.27]{ConstrictorPanel.png}
\vspace{-2.5em}
...
\paragraph{Can you do it on anything but the DX7?} Training an autoencoder requires a great many patches.  Most synths just don't have enough unique patches available online.  One obvious target might be the Yamaha TX81Z: but it's only got around 5000 unique patches, which might not be enough.  We'll see.
