...
\item {\it Light Gray with Black Lettering}\quad Patches in locations which ought not be written to or read from your synthesizer because they're {\bf inappropriate patch locations.}  You could still save these out to disk, however.  For example, the Proteus 2000 has four card slots which can be populated with any of 17 different cards.  If a card is populating a slot, its ``bank'' is displayed in Pale Magenta as normal.  If a card is not populating a slot, its ``bank'' is shown as Inappropriate: it's a real card with legitimate patch locations, so reading its patches from disk is reasonable, but reading them from the synthesizer won't do anything (because it's not installed).  You can treat these spots as scratch locations if you like.
...
You can close the Librarian by selecting {\bf Close Librarian} in the {\bf Edit} menu.  The data doesn't go away: if you reopen it, you'll be right back where you left off.
