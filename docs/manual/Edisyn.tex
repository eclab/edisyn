...
\item {\it Pale Magenta}\quad Paatches in locations which {\bf cannot} be written to your synthesizer because they're read-only on your machine.  However you can save them to disk, and can load them from the disk or from your synth.
\item {\it Light Gray with Red Lettering}\quad Patches in locations which {\bf cannot} be written to your synthesizer because they're invalid patch locations.  Why does Edisyn provide them then?  Because some synths have banks with varying lengths or present other unusual situations which violate the rectangular table structure used by Edisyn.  You can treat them as scratch locations.
\item {\it Dark Gray}\quad Patches you have selected.
\item {\it Pale Violet}\quad The drop location for drag-and-drop.
\end{itemize}
...
To save all patches in the Librarian  to a single file or collection of files, choose {\bf Save All Patches} from the {\bf Librarian} menu.  Some synthesizers can save out banks either with a single bank sysex message or as individual patch sysex messages.  Edisyn will give you the option if it can.  Note that if you select {\bf Save} or {\bf Save As...} from the {\bf File} menu while in the Librarian, it'll do the same thing as {\bf Save All Patches}.
