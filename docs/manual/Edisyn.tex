...
{\large \bf A Java-based Synthesizer Patch Editor and Librarian, Version 30\\[0.2em]
...
\item E-Mu Planet Phatt, Orbit V1 and V2, Carnaval, Vintage Keys, and Vintage Keys Plus
...
\item {\it Load} all of the patches matching a current synth type into the this editor's librarian or into a new librarian.  Note that if these are bank patches, their individual patches will be loaded into the librarian: and they may overwrite other bank patches also being loaded.  See Section~\ref{librarian} for more information about the librarian.
\end{itemize}
...
\item {\it Load} all of the patches in the bank into the this editor's librarian or into a new librarian.    See Section~\ref{librarian} for more information about the librarian.
\end{itemize}
...
For synthesizers which do not respond to individual parameter changes, Edisyn by default dumps the entire set of parameters to them when you change a parameter value.  However this will require a pause in-between dumps which you may not want.  In this case, you can turn this feature off by unchecking {\bf Sends Real Time Changes} (your selection is stored in Edisyn per-synthesizer, so different synth editors will check or not check this menu option).  You can always manually update your synthesizer by selecting {\bf Send to Current Patch}.
...
\paragraph{Mix}  This is an option only available in the Librarian's menu (Section~\ref{librarian}).  Here, you select two or more patches\,---\,often many\,---\, and Edisyn will use recombination to blend them all together to form a new random patch.  You have four options depending on how much you want to bias the result to favor the earlier (topmost) patch or patches.

\begin{itemize}
\item Mix Uniformly.  The final patch is a uniform combination of all the patches.
\item Mix Light Bias.  The final patch consists of 1/3 of the first (topmost) patch, with the remainder being 1/3 of the next patch, and so on.  The last two patches split their remainder fifty-fifty.
\item Mix Medium Bias.  The final patch consists of 1/2 of the first (topmost) patch, with the remainder being 1/2 of the next patch, and so on.  The last two patches split their remainder fifty-fifty.
\item Mix Heavy Bias.  The final patch consists of 2/3 of the first (topmost) patch, with the remainder being 2/3 of the next patch, and so on.  The last two patches split their remainder fifty-fifty.
...
\begin{wrapfigure}{r}{3.6in}
\vspace{-3em}\includegraphics[scale=0.27]{ConstrictorPanel.png}
\vspace{-3em}
...
\item {\it Pale Magenta}\quad Paatches in locations which cannot be written to your synthesizer because they're {\bf read-only} on your machine.  However you can save them to disk, and can load them from the disk or from your synth.
\item {\it Light Gray with Red Lettering}\quad Patches in locations which cannot be written to your synthesizer because they're {\bf invalid patch locations.}  Why does Edisyn provide them then?  Because some synths have banks with varying lengths or present other unusual situations which violate the rectangular table structure used by Edisyn.  You can treat them as scratch locations.
...
\paragraph{Exploration Tools} If you have selected a group of patches, you can route them to be Nudge targets, Morph targets, or the initial values of the Hill-Climber by choosing {\bf To Nudge Targets}, {\bf To Morph Targets}, or {\bf To Hill-Climber} respectively.   The Librarian also has its own unique exploration tool, {\bf mixing}, which lets you blend a potentially large number of patches into one single patch.  See the {\bf Mix} paragraph in Section~\ref{explorationtools} for discussion of this.
