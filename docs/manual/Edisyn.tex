...
Let's say you're adding a single (non-multimode) patch editor for the Yamaha DX7.  Here's what you'd do.
...
\begin{verbatim}
String key = "wavetable";  // the key in the model
String[] params = ARPEGGIATOR_CLOCK;  // this is an array of wavetable names elsewhere
JComponent comp = new Chooser("Wave Table", this, key, params);
\end{verbatim}

There's an option to add images to the chooser's menu.  Look it up.  Also see below (in the {\bf IconDisplay} bullet) for suggestions about how to load the images.
...
((LabelledDial)comp).setSecondLabel("MIDI Channel");
\end{verbatim}

In fact, this method is poorly named.  You can add additional (third, fourth, ...) labels with it!

It is very common that you need a custom string display for certain numbers in the center of the dial. You can do it like this:
...
\item {\bf EnvelopeDisplay}\quad This displays a wide variety of envelopes.  There are two main kinds of envelopes your synthesizer might display.  First, your synthesizer might define parameters (like attack) in terms of the {\it height} of the attack and also the {\it amount of time} necessary to reach that height.  This is intuitive to draw, but in fact many synthesizers don't do it that way.  Instead, some define it in terms of the {\it height} of the attack and the {\it rate of change} (or slope, or angle).  In the first case, the height of the attack has no bearing on how long it takes to reach it.  But in the second case, the amount of time to reach the attack depends on both the height and on the rate.  You will need to figure out what your synthesizer does exactly.  This is complicated by some synthesizers (like Yamaha's) which use rate, but compute it not in terms of angle, but in terms of (essentially) 90 degrees {\it minus} the angle.  Thus a steeper rate is a {\it lower number}.

Envelopes are drawn as a series of points, and between every successive pair of points we draw a line.    You will provide the EnvelopeDisplay with several arrays defining the coordinates of those points.  
...
What if your synth uses angles/rates/slopes rather than time intervals?  For example, the Waldorf Blofeld does this.\footnote{Actually, while the Blofeld uses the slope mechanism, its internal envelope displays on-board the synthesizer [incorrectly!] use the earlier-discussed mechanism.  So Edisyn is describing the envelopes more accurately than the Blofeld itself is!}   To handle this situation, we add an additional array of double constants called {\it angles}.   It works like this.  The height keys and height constants are exactly as before.  And {\tt timeConstants[0]} still defines the x position of the first point in the envelope, as before.  But the other time constants work differently.
...
Below is the Waldorf Blofeld's code for an ADSR envelope.  All the values go 0...127.  See if this makes sense now:
...
Envelopes generally stretch to fill all available space: they're particularly good to put as the ``last'' element in an HBox.  But you might want to fix them to a specific width. In this case, just call {\bf setPreferredWidth(...)}.

\end{itemize}

\paragraph{Spacing}

Occasionally you might need to add some fixed space to separate widgets.  See the {\tt Strut} class for factory methods that can build some struts for you.
...
First, you need to implement the {\bf recognize(...)} method.  This method tells Edisyn that you recognize a sysex message.  You should verify the message length and the header to determine that it is in fact meant for your type of synthesizer {\it and} is probably correct.  This method will also be called when loading a sysex file from disk.
...
It is possible that the parse(...) method will actually contain multiple sysex messages, if you loaded from a file.  This is because when emit a patch to the synth, one option you have (you'll see it later) is to send a stream of sysex messages; and when you write to a file, these same messages will get compacted into the file.  So if you do something fancy with emit(...) later, you may need to revise your parse(...) implementation.

You also need to implement the {\bf gatherPatchInfo(...)} method.  This method is nontrivial to implement.  Its function is to work with the user to determine the patch number, bank number, etc. necessary to ask the synthesizer for a given patch.  I suggest you take a look at existing patch editors to see how they have implemented it, and largely copy that.
...
You will also need to implement {\bf changePatch(...)} to issue a patch change (it'll be called as part of performRequestDump(...)).    It's possible that your synthesizer must pause for a bit after a patch change (the Blofeld, for example, requires almost 200ms).  You may want to implement the {\bf getPauseAfterChangePatch()} method to slow Edisyn down.
...
Note that emit(Model, ...) and emitAll(Model, ...) are also used to write out files.  If you implemented emitAll(...), be aware that Edisyn will strip out all of the javax.sound.midi.SimpleMessage messages and just pack together then remaining sysex messages.
