...
{\bf Request Next Patch} is similar to Request Patch... except that it asks the synthesizer to send the next patch number beyond the patch Edisyn is editing.  Note that if your synth is set to a different patch number than Edisyn thinks it's working on, then Edisyn's number is used.
