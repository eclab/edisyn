...
\item {\it Light Gray with Black Lettering}\quad Patches in locations which ought not be written to or read from your synthesizer because they're {\bf inappropriate patch locations.}  You could still save these out to disk, but the synthesizer likely will not respond them if you try to request or write them.  For example, the Proteus 2000 has four card slots which can be populated with any of 20 different read-only cards.  If a card is populating a slot, its ``bank'' is displayed in Pale Magenta as normal.  If a card is not populating a slot, its ``bank'' is shown as Inappropriate: it's a real card type with legitimate patch locations, and which {\it could} be installed on a synth, so reading/writing its patches to the disk is reasonable, but accessing them on your synthesizer won't do anything (because it's not installed on your machine).  You can treat these spots as scratch locations if you like.
